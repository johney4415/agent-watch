...
- The Refresh icon briefly changes to a checkmark after reconciliation. Completed and failed rows are acknowledged and removed after you open them.
- Historical Claude `idle_prompt` events created by older Agent Watch versions are reclassified from `Needs input` to `Idle` when loaded.
- Selecting a session closes the menu-bar panel and transfers app and keyboard focus to the matching terminal.
